    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for(var i=0;i<device.length;i++){
                ...
                el+= "<a href='/pages/device.php?nome="+device[k].A+"&tipo="+device[k].F+"'><button id='back' type='button' class='btn btn-default btn-md'><span class='glyphicon glyphicon-menu-left' aria-hidden='true'></span></button></a>                                                                                                 <a href='/pages/device.php?nome="+device[j].A+"&tipo="+device[j].F+"'><button id='next' type='button' class='btn btn-default btn-md'><span class='glyphicon glyphicon-menu-right' aria-hidden='true'></span></button></a>                                                                                                       <div class='col-md-9 col-xs-11'><img class='immagine blocco img-responsive left' id='dev' src='"+device[i].G+"'> <img class='immagine blocco img-responsive left'  src='"+device[i].G2+"'><button type='button' class='btn btn-default btn-sm guide'><img href='#' src='/img/device/onlineguide.png' aria-hidden='true'></button></div><h2 id='titolo'>"+device[i].A+"</h2> <div id='features' class='col-md-3 col-xs-12'><h3 id='sottotitolo''>Features</h3><p class='paragrafi'>"+device[i].C+"</p> <h4 class='prezzo-dev'>"+device[i].D+"</h4> </div> <div id='desc' class='col-md-12 col-xs-11'><h3 id='sottotitolo'>Description</h3><p class='paragrafi'>"+device[i].B+"</p></div>";             
                }
            }
